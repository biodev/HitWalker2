...
setClass(Class="BasicGenotypes", representation=list(geno.sample="data.frame", geno.gene="data.frame"), contains="NeoData",
          prototype=list(
                        ...
                        WITH $$lower_ret_type$$.name AS ret_type, COLLECT(DISTINCT $$lower_coll_type$$.name) AS use_coll WHERE LENGTH(use_coll) = {$$coll_type$$_length} RETURN DISTINCT ret_type',
...
main <- function()
{
    ...
    #cd ~/HitWalker2
    #./change_hw2_instance.sh tmpl
    
    hwhelper:::make.graph.struct(ccle.graph, graph.struct.path="/var/www/hitwalker2_inst/static/network/data/graph_struct.json")
}
