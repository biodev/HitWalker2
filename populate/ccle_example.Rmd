...
#without specifying a subject to sample mapping in readMaf.ccle, the below check should be true
...
As before we still need to supply the mapping between the CEL file names and the subject names and we will do this base on the SIF file instead of relying on the samples and subjects to be named the same.  On generating the `HW2exprSet` object we will supply it a `data.frame` with the mapping info.  Using `addSamples` with this object will produce the correct subject to sample mapping.
...
names(expr.annot) <- c("subject","sample")

hw2.ccle.eset <- HW2exprSet(ccle.eset, sample.mapping = expr.annot)

addSamples(subj) <- hw2.ccle.eset
...
hw2.conf <- makeHW2Config(subject=subj, gene.model="entrez",
        ...
        Expression=hw2.ccle.eset, GeneScore=drug, Variants=maf.obj)
