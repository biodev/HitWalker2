...
CCLE\_NP24.2009\_Drug\_data\_2012.02.20.csv	
...
populate(hw2.conf, neo.path=normalizePath("neo4j-community-2.1.6/"))
...
ccle.graph <- compute.graph.structure(neo.path=normalizePath("neo4j-community-2.1.6/"))
...
Note that addition info can be added to the config and custom_functions files manually, for instance for the CCLE data one can add in a link to the CCLE website on the sample node right click by specifying:

node_content['Sample']['args']['sample\_link'] = 'http://www.broadinstitute.org/ccle/cell lines/'

Additionally, node and edge abbreviations can be specified by adding in dictionaries in the form: node_abbreviations={'current\_name':'desired\_name'}
...
These node names should correspond to the node types or the values in node_abbreviations, note that only node *\_Hits and metadata/attributes should be specified here.
