...
system("wget ftp://string-db.org/STRING/9.1/mapping_files/Entrez_mappings/entrez_gene_id.vs.string.v9.05.28122012.txt")
    ...
    system.time(load.neo4j(.data=string.ppi, edge.name="ASSOC", commit.size=10000L, neo.path="/Users/bottomly/Desktop/hitwalker2_paper/neo4j-community-2.1.6", dry.run=F, unique.rels=F))
    
    #user   system  elapsed 
    #5.944    0.990 1400.100 
    ...
    ##example of what the format should look like:  data.frame(node1, node2, node[12].property, edge property(no x'.'y just name))
    
    #also make the *.mtx and .names matrix files for string
    
    require(igraph)
    require(Matrix)

    string.graph <- read.delim("9606.protein.links.v9.1.txt.gz", sep="", header=T, stringsAsFactors=FALSE)
